...
{
    return (cell == 'E' || cell == 'C' || cell == 'P' || cell == '1' || cell == '0');
}
    ...
    while (y < game->height)
    {
        ...
        while (x < game->width)
        {
            ...
            if (!is_valid_char(cell))
                return (write(1, "Error: Invalid character", 24));
            x++;
        }
        y++;
    }
...
int check_cpe01(t_game *game)
{
    ...
    {
        write(1, "Error: Map must contain at least one 'E', 'C', 'P', '1', and '0'\n", 65);
        return (0);
    }
...
{
    int fd = open(file, O_RDONLY | __O_DIRECTORY);
    if (fd == -1) {
        return (1);
    }
    close(fd);
...
int is_ber(const char *filename)
{
    ...
    if (strcmp(filename + len_filename - len_extension, extension) != 0)
        return (0);
    if (len_filename == len_extension || filename[len_filename - len_extension - 1] == '.')
        return (0);
    return (1);
}
